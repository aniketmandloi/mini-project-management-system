...
class User(AbstractUser):
    ...
    is_organization_admin = models.BooleanField(
        default=False, help_text="Whether this user is an admin for their organization"
    )
    last_login_ip = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address of the last login"
    )
...
class EmailVerificationToken(models.Model):
    ...
    email = models.EmailField(
        validators=[EmailValidator()], help_text="Email address to verify"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    expires_at = models.DateTimeField(help_text="When this token expires")
    used_at = models.DateTimeField(
        null=True, blank=True, help_text="When this token was used"
    )
